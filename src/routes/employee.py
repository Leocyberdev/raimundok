...
def service_orders():
    ...
    # Buscar ordens de serviço atribuídas ao funcionário atual
...
def download_service_order_pdf(service_order_id):
    ...
    if not service_order.pdf_filename:
        flash('Esta ordem de serviço não possui arquivo PDF.', 'error')
    ...
    pdf_path = os.path.join(pdf_dir, service_order.pdf_filename)
    ...
    try:
        return send_from_directory(pdf_dir, service_order.pdf_filename, as_attachment=True)
    except Exception as e:
        flash('Erro ao baixar o arquivo PDF.', 'error')
        return redirect(url_for('employee.service_order_detail', service_order_id=service_order_id))
