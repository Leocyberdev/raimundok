...
from models.user import db, User, AuditLog, FileReference
...
from database.config import get_database_config, is_production
...
from utils.date_utils import format_date_pt, format_bytes
...
# Configurações de sessão para funcionar corretamente
app.config['SESSION_COOKIE_SAMESITE'] = None  # Permite cookies em requisições cross-site
app.config['SESSION_COOKIE_SECURE'] = False
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_NAME'] = 'session'
app.config['REMEMBER_COOKIE_SECURE'] = False
app.config['REMEMBER_COOKIE_HTTPONLY'] = True
app.config['REMEMBER_COOKIE_DURATION'] = 86400  # 24 horas
...
with app.app_context():
    try:
        ...
        try:
            ...
            if 'description' not in [col['name'] for col in inspector.get_columns('order')]:
                ...
                print("Coluna description adicionada com sucesso!")
                        ...
                        # Migrar coluna tracking_token
            if 'tracking_token' not in [col['name'] for col in inspector.get_columns('order')]:
                print("Adicionando coluna tracking_token...")
                db.session.execute(text('ALTER TABLE "order" ADD COLUMN tracking_token VARCHAR(100) UNIQUE'))
                db.session.commit()
                print("Coluna tracking_token adicionada com sucesso!")
        ...
        try:
            ...
            db.session.commit()
            print("Usuários admin padrão criados/verificados com sucesso!")
        except Exception as e:
